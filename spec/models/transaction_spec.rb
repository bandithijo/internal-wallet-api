  ...
  describe 'Class Methods' do
    ...
    context '.perform_transaction' do
      it 'successfull performs a transaction and updates wallet balances' do
        expect { Transaction.perform_transaction(source_wallet: source_wallet, target_wallet: target_wallet, amount: 500) }
      ...
      it 'raise an error when there are sufficient funds' do
        expect { Transaction.perform_transaction(source_wallet: source_wallet, target_wallet: target_wallet, amount: 1500) }
      ...
      it 'creates a transaction with only a source wallet (withdrawals or debiting a wallet)' do
        expect { Transaction.perform_transaction(source_wallet: source_wallet, amount: 200) }
      ...
      it 'creates a transaction with only a target wallet (deposits or crediting a wallet)' do
        expect { Transaction.perform_transaction(target_wallet: target_wallet, amount: 300) }
